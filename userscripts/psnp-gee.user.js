...
exports.guideID = exports.psnpGeeCustomClassName = void 0;
const editColors_1 = __webpack_require__(590);
const gridMoving_1 = __webpack_require__(456);
const selfLinkImage_1 = __webpack_require__(647);
const autoSave_1 = __webpack_require__(596);
exports.psnpGeeCustomClassName = "psnp-gee-editor";
main();
function main() {
...
function parseGuideID() {
    ...
    if (match) {
        exports.guideID = Number(match[1]);
    }
    else {
...
function setupObserver() {
    const observer = new MutationObserver(() => {
        (0, gridMoving_1.addGridToolbarButtonsIfNeeded)();
        (0, editColors_1.addColorButtonsIfNeeded)();
        (0, selfLinkImage_1.addImageButtonsIfNeeded)();
        listenForEditors();
    });
...
function listenForEditors() {
    ...
    for (const editor of editors) {
        if (editor.classList.contains(exports.psnpGeeCustomClassName)) {
            continue;
        }
        editor.className = `${editor.className} ${exports.psnpGeeCustomClassName}`;
        (0, selfLinkImage_1.onEditorInitializationForImage)(editor);
        (0, autoSave_1.onEditorInitializationForAutoSave)(editor);
    }
}
...
exports.triggerContentChangedEvent = triggerContentChangedEvent;
exports.saveStateToUndoStack = saveStateToUndoStack;
exports.getFroalaEditor = getFroalaEditor;
exports.findEditorInfo = findEditorInfo;
var EditorType;
(function (EditorType) {
    EditorType["Roadmap"] = "roadmap";
    EditorType["Section"] = "section";
})(EditorType || (exports.EditorType = EditorType = {}));
function triggerContentChangedEvent(view) {
...
function saveStateToUndoStack(view, onlyIfUndoStackEmpty) {
...
function getFroalaEditor(view) {
    const editorInfo = findEditorInfo(view);
    if (!editorInfo) {
        return null;
    }
    const $editor = $(`#${editorInfo.fullEditorID}`);
    return $editor.data("froala.editor");
}
function findEditorInfo(startElement) {
...
function getNumericSectionID(fullEditorID) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.addGridToolbarButtonsIfNeeded = addGridToolbarButtonsIfNeeded;
const froalaHelpers_1 = __webpack_require__(263);
function addGridToolbarButtonsIfNeeded() {
    const toolbars = Array.from(document.getElementsByClassName('fr-buttons'));
    toolbars.forEach(toolbar => {
        const firstButton = toolbar.firstChild;
        if (!(firstButton === null || firstButton === void 0 ? void 0 : firstButton.id.startsWith("tableHeader-"))) {
            return;
        }
        const dropDownMenus = Array.from(toolbar.getElementsByClassName('fr-dropdown-menu'));
        dropDownMenus.forEach(menu => processGridDropdownMenu(menu));
    });
}
function processGridDropdownMenu(menu) {
...
function appendRowButtons(list) {
    const moveUpNode = cloneButton(list.children[0], "Move row up", () => moveSelectedRow(true));
    const moveDownNode = cloneButton(list.children[0], "Move row down", () => moveSelectedRow(false));
    list.append(moveUpNode, moveDownNode);
}
function appendColumnButtons(list) {
    const moveLeftNode = cloneButton(list.children[0], "Move column left", () => moveSelectedColumn(true));
    const moveRightNode = cloneButton(list.children[0], "Move column right", () => moveSelectedColumn(false));
    list.append(moveLeftNode, moveRightNode);
}
function cloneButton(node, title, onClick) {
...
function moveSelectedRow(up) {
    ...
    if (up && (tbody === null || tbody === void 0 ? void 0 : tbody.firstChild) === row) {
        return;
    }
    if (!up && (tbody === null || tbody === void 0 ? void 0 : tbody.lastChild) === row) {
        return;
    }
    (0, froalaHelpers_1.saveStateToUndoStack)(selectedCell, true);
    if (up) {
        const prev = row.previousElementSibling;
        if (prev) {
            tbody === null || tbody === void 0 ? void 0 : tbody.insertBefore(row, prev);
        }
    }
    else {
        const next = row.nextElementSibling;
        if (next) {
            tbody === null || tbody === void 0 ? void 0 : tbody.insertBefore(next, row);
        }
    }
    (0, froalaHelpers_1.saveStateToUndoStack)(selectedCell, false);
    (0, froalaHelpers_1.triggerContentChangedEvent)(selectedCell);
}
function moveSelectedColumn(left) {
    const selectedCell = getSelectedCell();
    if (!selectedCell) {
        return;
    }
    const row = selectedCell.parentElement;
    const tbody = row === null || row === void 0 ? void 0 : row.parentElement;
    const table = tbody === null || tbody === void 0 ? void 0 : tbody.parentElement;
    if (!tbody) {
        return;
    }
    if (!table) {
...
function moveTableCell(table, tbody, from, to) {
    var _a;
    (0, froalaHelpers_1.saveStateToUndoStack)(table, true);
    const headerRow = (_a = table.tHead) === null || _a === void 0 ? void 0 : _a.rows[0];
    if (headerRow) {
        headerRow.insertBefore(headerRow.cells[from], to < from ? headerRow.cells[to] : headerRow.cells[to].nextSibling);
    }
    Array.from(tbody.children).forEach(r => {
        const row = r;
        row.insertBefore(row.cells[from], to < from ? row.cells[to] : row.cells[to].nextSibling);
    });
    (0, froalaHelpers_1.saveStateToUndoStack)(table, false);
    (0, froalaHelpers_1.triggerContentChangedEvent)(table);
}
...
var cachedCustomColors;
function getCustomColors(guideID) {
    var _a;
    if (!cachedCustomColors) {
        loadCustomColors();
    }
    return (_a = cachedCustomColors === null || cachedCustomColors === void 0 ? void 0 : cachedCustomColors.guideIDToCustomColorList.get(guideID)) !== null && _a !== void 0 ? _a : [];
}
function setCustomColors(guideID, colors) {
    if (!cachedCustomColors) {
        loadCustomColors();
    }
    cachedCustomColors === null || cachedCustomColors === void 0 ? void 0 : cachedCustomColors.guideIDToCustomColorList.set(guideID, colors);
    saveCustomColors();
}
function loadCustomColors() {
    ...
    if (customColorsJSON) {
        ...
        cachedCustomColors = {
            guideIDToCustomColorList: new Map(parsed.guideIDToCustomColorList || [])
        };
    }
    else {
        cachedCustomColors = {
            guideIDToCustomColorList: new Map()
        };
    }
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.addColorButtonsIfNeeded = addColorButtonsIfNeeded;
const sweetalert2_1 = __importDefault(__webpack_require__(465));
const index_1 = __webpack_require__(156);
const froalaHelpers_1 = __webpack_require__(263);
const customColors_1 = __webpack_require__(475);
const defaultColors = [
    "#41A85F",
    "#2C82C9",
    "#9365B8",
...
function addColorButtonsIfNeeded() {
...
function rebuildColorPanel(createdEditColorsButton = null) {
    const textColorDiv = getTextColorDiv();
    const clearFormattingButton = getClearFormattingButton(textColorDiv);
    const colorButtonTemplate = getColorButton(textColorDiv);
    const editColorsButton = createdEditColorsButton !== null && createdEditColorsButton !== void 0 ? createdEditColorsButton : getEditColorsButton(textColorDiv);
    if (!textColorDiv || !editColorsButton || !clearFormattingButton || !colorButtonTemplate || !index_1.guideID) {
        return;
    }
    Array.from(textColorDiv.childNodes).forEach(childNode => childNode.remove());
    const fullColorList = defaultColors.concat((0, customColors_1.getCustomColors)(index_1.guideID));
...
function openColorPanel(view) {
    const froalaEditor = (0, froalaHelpers_1.getFroalaEditor)(view);
    if (!froalaEditor) {
        return;
    }
    froalaEditor.colors.showColorsPopup();
}
function createEditColorsButton(textColorDiv) {
    const clearFormattingButton = getClearFormattingButton(textColorDiv);
    if (!clearFormattingButton) {
        console.log("Could not find Clear Formatting button");
        return null;
    }
    const editColorsButton = clearFormattingButton.cloneNode(true);
    ...
    const innerIcon = editColorsButton.childNodes[0];
    innerIcon.className = "fa fa-pencil";
    const innerSpan = editColorsButton.childNodes[1];
    innerSpan.innerText = editColorsTitle;
    return editColorsButton;
}
function createColorButtonForColor(color, colorButtonTemplate) {
    const colorButton = colorButtonTemplate.cloneNode(true);
    colorButton.style.backgroundColor = color;
    colorButton.setAttribute("data-param1", color.toUpperCase());
    const innerSpan = colorButton.childNodes[0];
    innerSpan.innerText = `Color ${color.toUpperCase()}   `;
    return colorButton;
}
function getTextColorDiv() {
    const textColorDivs = Array.from(document.getElementsByClassName('fr-text-color'));
    if (textColorDivs.length == 0) {
        return null;
    }
    return textColorDivs[0];
}
function getColorButton(textColorDiv) {
    var _a;
    return Array.from((_a = textColorDiv === null || textColorDiv === void 0 ? void 0 : textColorDiv.childNodes) !== null && _a !== void 0 ? _a : [])
        .find(x => !x.classList.contains("fr-active-item"));
}
function getClearFormattingButton(textColorDiv) {
    var _a, _b;
    return (_b = Array.from((_a = textColorDiv === null || textColorDiv === void 0 ? void 0 : textColorDiv.childNodes) !== null && _a !== void 0 ? _a : [])
        .map(x => x)
        .find(x => x.getAttribute("data-param1") === "REMOVE")) !== null && _b !== void 0 ? _b : null;
}
function getEditColorsButton(textColorDiv) {
    var _a, _b;
    return (_b = Array.from((_a = textColorDiv === null || textColorDiv === void 0 ? void 0 : textColorDiv.childNodes) !== null && _a !== void 0 ? _a : [])
        .map(x => x)
        .find(x => x.title === editColorsTitle)) !== null && _b !== void 0 ? _b : null;
}
function showEditColorsPopup(view) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!index_1.guideID) {
            return;
        }
        const colors = (0, customColors_1.getCustomColors)(index_1.guideID);
        const { value: updatedColors } = yield sweetalert2_1.default.fire({
        ...
        if (updatedColors) {
            (0, customColors_1.setCustomColors)(index_1.guideID, updatedColors);
            rebuildColorPanel();
            openColorPanel(view);
        }
    });
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.onEditorInitializationForAutoSave = onEditorInitializationForAutoSave;
...
var editorIDToLastSavedDate = new Map();
function onEditorInitializationForAutoSave(editor) {
    setupQuickSaveButton(editor);
    listenForSaveShortcut(editor);
}
function setupQuickSaveButton(editor) {
    var _a, _b;
    const editorInfo = (0, froalaHelpers_1.findEditorInfo)(editor);
    if (!editorInfo) {
    ...
    const froalaEditor = (0, froalaHelpers_1.getFroalaEditor)(editor);
    if (!froalaEditor) {
        return;
    }
    ...
    if (((_a = parentDiv.firstElementChild) === null || _a === void 0 ? void 0 : _a.tagName) === "SPAN") {
        console.log(parentDiv.firstElementChild);
        const stageName = parentDiv.firstElementChild;
    ...
    if (sectionRevisions.length == 1) {
        const sectionRevision = sectionRevisions[0];
        ...
        sectionRevision.firstElementChild.style.flex = "1";
        sectionRevision.firstElementChild.style.minWidth = "0";
        sectionRevision.firstElementChild.style.flexGrow = "1";
        sectionRevision.firstElementChild.style.width = "100%";
    }
    buttonContainer.style.display = "flex";
    buttonContainer.style.flexWrap = "nowrap";
    buttonContainer.style.gap = "4px";
    buttonContainer.style.margin = "0";
    buttonContainer.style.marginBottom = editorInfo.editorType == froalaHelpers_1.EditorType.Roadmap ? "10px" : "0px";
    ...
    const quickSaveButton = saveButton.cloneNode(true);
    ...
    (_b = saveButton.parentElement) === null || _b === void 0 ? void 0 : _b.insertBefore(quickSaveButton, saveButton.nextElementSibling);
}
function listenForSaveShortcut(editor) {
...
function save(parentDivView, editorDiv) {
    return __awaiter(this, void 0, void 0, function* () {
        const froalaEditor = (0, froalaHelpers_1.getFroalaEditor)(editorDiv);
        const editorInfo = (0, froalaHelpers_1.findEditorInfo)(editorDiv);
        const saveButtons = Array.from(parentDivView.getElementsByClassName(customSaveButtonClass));
        if (!froalaEditor || !editorInfo || !__1.guideID || saveButtons.length == 0) {
            console.log("Unable to find required information to save.");
            return;
        }
        const quickSaveButton = saveButtons[0];
        quickSaveButton.disabled = true;
        quickSaveButton.innerText = "Saving...";
        const nameInput = parentDivView.querySelector('input');
        ...
        if (editorInfo.editorType == froalaHelpers_1.EditorType.Roadmap) {
...
function findSectionContainerDivView(editorInfo) {
    if (editorInfo.editorType == froalaHelpers_1.EditorType.Roadmap) {
        return document.getElementById(`roadmapStep${editorInfo.numericEditorID}`);
    }
    else {
        return document.getElementById(`SectionContainer${editorInfo.numericEditorID}`);
    }
}
function addOrUpdateSaveLabel(parentDivView, editorInfo) {
    var _a, _b;
    ...
    if (labels.length == 1) {
        ...
        (_a = quickSaveButton.parentElement) === null || _a === void 0 ? void 0 : _a.insertBefore(label, (_b = quickSaveButton.parentElement) === null || _b === void 0 ? void 0 : _b.firstChild);
    }
    const updateText = () => {
    ...
    const observer = new MutationObserver(() => {
        ...
        if (!document.body.contains(label) || parentDivView.firstElementChild.style.display === "none") {
            if (timeoutID) {
...
function performRequest(options) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const response = yield fetch((options.url.startsWith('/') ? '' : '/xhr/') + options.url, {
                method: options.method,
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
                    'x-csrf-token': getCSRFToken()
                },
                body: options.method === 'POST' && options.data
...
function formattedTimeAgo(fromDate) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.onEditorInitializationForImage = onEditorInitializationForImage;
exports.addImageButtonsIfNeeded = addImageButtonsIfNeeded;
const froalaHelpers_1 = __webpack_require__(263);
const index_1 = __webpack_require__(156);
const selfLinkImageTitle = "Self Link Image";
var selectedImage;
function onEditorInitializationForImage(editor) {
...
function addImageButtonsIfNeeded() {
    ...
    const buttonContainer = popupContainer.lastElementChild;
    if (Array.from(buttonContainer.childNodes).some(x => x.classList.contains(index_1.psnpGeeCustomClassName))) {
        return;
    }
    ...
    const selfLinkButton = imageRemoveNode.cloneNode(true);
    selfLinkButton.className = `${selfLinkButton.className} ${index_1.psnpGeeCustomClassName}`;
    ...
    const innerI = selfLinkButton.childNodes[0];
    innerI.className = "fa fa-arrows-alt";
    const innerSpan = selfLinkButton.childNodes[1];
...
function selfLinkSelectedImage() {
    ...
    (0, froalaHelpers_1.saveStateToUndoStack)(selectedImage, true);
    if (parent && parent.tagName === 'A') {
        if (parent.href === imageURL) {
            (_a = parent.parentElement) === null || _a === void 0 ? void 0 : _a.replaceChild(selectedImage, parent);
        }
        else {
        ...
        parent === null || parent === void 0 ? void 0 : parent.replaceChild(anchor, selectedImage);
        anchor.appendChild(selectedImage);
    }
    (0, froalaHelpers_1.saveStateToUndoStack)(selectedImage, false);
    (0, froalaHelpers_1.triggerContentChangedEvent)(selectedImage);
    selectedImage === null || selectedImage === void 0 ? void 0 : selectedImage.click();
}
